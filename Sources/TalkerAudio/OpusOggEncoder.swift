...
public class OpusOggEncoder: AudioEncoderProtocol {
    ...
    public convenience init(
        format: AudioStreamBasicDescription, opusRate: Int, application: Application
    ) throws {
        try self.init(
            pcmRate: Int(format.mSampleRate),
            pcmChannels: Int(format.mChannelsPerFrame),
    ...
    public init(
        pcmRate: Int, pcmChannels: Int, pcmBytesPerFrame: UInt32, opusRate: Int,
        application: Application
    ) throws {
        // avoid resampling
        ...
        encoder = opus_encoder_create(Int32(opusRate), Int32(pcmChannels), application.rawValue, &status)
    ...
    public func encode(inputBuffer pcmBuffer: AVAudioPCMBuffer) throws -> Data {
        let buf = pcmBuffer.int16ChannelData!.pointee
        try encode(pcm: buf, count: Int(pcmBuffer.totalBytes()))
        return bitstream(flush: false)
    }
    ...
    public func encode(pcm: Data) throws {
        try pcm.withUnsafeBytes { rawBuffer in
            guard let bytes = rawBuffer.bindMemory(to: Int16.self).baseAddress else { return }
    ...
    internal func encode(pcm: UnsafePointer<Int16>, count: Int) throws {
        ...
        while count >= Int(frameSize) * Int(pcmBytesPerFrame) {
            ...
            let status = opus.withUnsafeMutableBufferPointer { buffer -> Int32 in
    ...
    private func encodeCache(pcm: inout UnsafeMutablePointer<Int16>, bytes: inout Int) throws {
        ...
        try pcmCache.withUnsafeBytes { rawBuffer in
            guard let cache = rawBuffer.bindMemory(to: Int16.self).baseAddress else { return }
...
private class CommentHeader {
    ...
    init() {
        ...
        vendorString = String(validatingCString: opus_get_version_string())!
